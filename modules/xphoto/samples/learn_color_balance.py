...
def generate_code(model, input_params, use_YML, out_file):
    ...
    if use_YML:
        ...
        for key in input_params:
            res += f" {key} {input_params[key]}"
        res += "\n */\n"
        res += "const int num_features = 4;\n"
        res += f"const int _num_trees = {len(model)}" + ";\n"
        res += f"const int _num_tree_nodes = {str(2**depth)}" + ";\n"
...
if __name__ == '__main__':
    ...
    for i, file in enumerate(img_files):
        if (i>=img_range[0] and i<img_range[1]) or (img_range[0]==img_range[1]==0):
            ...
            for _ in range(int(args.num_augmented)):
